...
export function PromptComposer({
  ...
  const attachmentApi = attachmentBackend === 'antigravity' ? getAgentDock().antigravity : getAgentDock().codex

  async function saveFromPath(sourcePath: string): Promise<void> {
    if (!sessionId) return
    const result = await attachmentApi.saveAttachmentFromPath(sessionId, sourcePath)
  ...
  async function saveFromFile(file: File): Promise<void> {
    ...
    const result = await attachmentApi.saveAttachmentFromDataUrl(sessionId, dataUrl)
    if (result.path) setPendingImages((prev) => [...prev, result.path as string])
    else setAttachError(result.error ?? 'Could not attach this image.')
  }
  ...
  async function handleAttachClick(): Promise<void> {
    ...
    setAttaching(true)
    try {
      const paths = await attachmentApi.browseAttachments()
