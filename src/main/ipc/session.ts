...
export function ensureForwarding(window: BrowserWindow, sessionId: string): void {
  ...
  sessionService.onEvent(sessionId, (event) => {
...
export function registerSessionIpc(window: BrowserWindow): void {
  ...
  ipcMain.handle(IpcChannels.sessionSendPrompt, async (_event, sessionId: string, text: string) => {
    ensureForwarding(window, sessionId)
    await sessionService.sendPrompt(sessionId, text)
  })
