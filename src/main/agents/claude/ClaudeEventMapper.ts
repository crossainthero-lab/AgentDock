// Maps `claude -p --output-format stream-json --verbose
// --include-partial-messages`'s newline-delimited JSON directly into the
// shared AgentEvent vocabulary. No screen reconstruction, no line-index
// bookkeeping, no reflow — every line is a self-describing typed object.
//
// Shapes below are grounded in real output captured against the installed
// `claude 2.1.211` (see repo session notes), not guessed:
...
//
// One CLI invocation (one AgentDock turn) can contain multiple internal
// Claude API turns when tools are used (confirmed live: a single tool-use
// prompt produced `"num_turns":2` with two separate message_start/
// message_stop cycles) — so a turn legitimately produces multiple
// assistant_completed events, one per message_id. This mapper doesn't
// collapse them; the reducer already supports N assistant messages per turn.
//
...
// UI for no visible benefit. A `tool_use` block becomes one activity_started
// (on content_block_start) + one activity_completed (on content_block_stop).
...
function mapParsed(obj: Record<string, unknown>, prev: ClaudeMapperState, sessionId: string, turnId: string): ClaudeMapResult {
  ...
  if (type === 'system' && obj.subtype === 'init') {
    ...
    return { events: [{ ...base, type: 'turn_started' }], state, capturedSessionId }
  }
  ...
  if (type === 'result') {
    ...
    if (isError) {
      const reason = typeof obj.result === 'string' && obj.result ? obj.result : 'Claude reported an error for this turn.'
